...
pno int primary key atuo_increment,
...
-- 추가
alter table product
add writer varchar(100) after madeby;
...
alter table replies
add constraint fk_replies_replier
foreign key (replier) references member(email)
on update cascade
on delete no action;

alter table replies
add constraint fk_replies_pno
