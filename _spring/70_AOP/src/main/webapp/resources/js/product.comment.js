...
// 서버로부터 댓글 리스트 받아오기
async function getCommentListFromServer(pnoVal, pageNo) {
    try {
...
function printCommentList() {
    ...
    getCommentListFromServer(pnoVal, pageNo=1).then(result => {
        console.log(result);
...
// 서버로 삭제할 댓글 전달하기
async function commentDeleteToServer(cnoVal, pnoVal){
    try {
        const url = "/comment/" + cnoVal + "/" + pnoVal;
...
document.addEventListener('click', (e) => {
    ...
    if (e.target.classList.contains('cmtDelBtn')) {
        ...
        commentDeleteToServer(cnoVal, pnoVal).then(result => {
            if (parseInt(result)) {
                alert('댓글이 삭제되었습니다');
            }
            printCommentList();
    ...
    if (e.target.classList.contains('moreBtn')) {
        ...
        let pnoVal = document.getElementById('pnoVal').innerText;
        getCommentListFromServer(pnoVal, pageNo).then(result => {
            console.log(result);
            if (result.cmtList.length > 0) {
                spreadCommentList(result);
            }
