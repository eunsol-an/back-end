...
public class FileHandler {
	...
	public List<FileVO> getFileList(MultipartFile[] files) {
		...
		today = today.replace("-", File.separator); // 2022\07\22(win), 2022/07/22(linux, mac) 이런식으로 바뀜
		
		String dirName = "product_file" + File.separator + today;
		
		File folder = new File(UP_DIR, dirName);
		...
		for (MultipartFile file : files) {
			...
			fvo.setSaveDir(dirName);
			fvo.setFileSize(file.getSize());
			
			String originalFileName = file.getOriginalFilename();
			String onlyFileName = originalFileName.substring(
					originalFileName.lastIndexOf("\\")+1); // 경로 제거 후 순수파일.확장자 이름만
			...
			try {
				...
				log.debug(">>> File 객체 물리디시스크에 저장 실패");
				e.printStackTrace();
			}
			fileList.add(fvo);
		}
	...
	public FileVO getFile(MultipartFile file) {
		LocalDate date = LocalDate.now();
		String today = date.toString(); // 2022-07-22
		today = today.replace("-", File.separator); // 2022\07\22(win), 2022/07/22(linux, mac) 이런식으로 바뀜
		
		String dirName = "member_img" + File.separator + today;
